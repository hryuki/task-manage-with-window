...
export function getAllTasks(): Task[] {
    ...
    SELECT id, name, parentId, "order", createdAt, updatedAt
    FROM tasks
    ORDER BY "order" ASC
  `);
...
export function createTask(name: string, parentId: string | null = null): Task {
  ...
  `);
    ...
    const stmt = db.prepare(`
    INSERT INTO tasks (id, name, parentId, "order", createdAt, updatedAt)
    VALUES (?, ?, ?, ?, ?, ?)
  `);

    stmt.run(id, name, parentId, maxOrder + 1, now, now);
...
export function updateTask(id: string, updates: Partial<Pick<Task, 'name' | 'parentId' | 'order'>>): Task | null {
    if (!db) throw new Error('Database not initialized');
